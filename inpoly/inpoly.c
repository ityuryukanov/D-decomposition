/*
 ...
 ----------
   mex  inpoly.c
   mex  -output inpoly.dll inpoly.c
   mex  -v -f mexopts_intel10.bat -output inpoly.dll inpoly.c
 
 
 Author
 ------
 
 Sébastien PARIS (sebastien.paris@lis-lab.fr)
...
void mexFunction(int nlhs, mxArray** plhs ,  int nrhs, const mxArray** prhs)
{
    ...
    if(mxGetM(prhs[1]) != 2)
    {
        mexErrMsgTxt("node must be (2 x Nnode)");   
    }
